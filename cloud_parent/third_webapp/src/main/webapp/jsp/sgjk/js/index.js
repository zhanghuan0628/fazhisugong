        ...
        success: function(data) {
            if(data.success){
                if(data.code == "2000"){
                    ...
                    var redirectUri =encodeURI("http://"+window.location.host+"/third/user/get_openid/"+appId+"/sgHealthy", "UTF-8");
...
function praiseToast(){
    if ($(".praiseToast").length == 0) {
        var praiseToastCase = "<div class='praiseToast'><p>今日10次助力次数以全用完，</p><p>我们明天继续来为健康助力！</p></div>";
        $("body").append(praiseToastCase);
        setTimeout(function(){
            $(".praiseToast").remove();
            $("#layer").hide();
        },2000);
...
function waterToast(txt,left,top){
    var waterToastCase = "<div class='waterToast' style='left:"+left+"px;top:"+top+"px'>健康值+"+txt+"</div>";
    $("body").append(waterToastCase);
    setTimeout(function(){
        $(".waterToast").remove();
    },2000);
}
...
function hasEnd(){
    if (endTime-new Date().getTime()<=0) {
        location.href = apiUrl+"/jsp/sgjk/index.jsp";
        return false;
    }
}

var endTime = 1525942800000;
// var endTime = 1524110340000
...
$(function(){
	...
	$("body").on("click",".toastCase a",function(){
		$(".toastLayer,.toastCase").remove();
	})
