...
sgRisk.initColumn = function () {
    var columns = [
        ...
        {title:'操作', render: function(data, type, row, meta){
        	var msg = "";
        	msg+='<a title="查看" href="javascript:;" onclick="sgRisk.user_ask_detail(\'查看详情\',\'/sg_user/user_ask_detail\','
			+ "'"
			+row.id
			+ "'"
			+',\'800\',\'500\')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe60c;</i>'
			+'</a> <a title="回复" href="javascript:;" onclick="sgRisk.back(\'咨询师回复\',\'/sg_user/counselor_back\','
			+ "'"
			+row.id
			+ "'"
