...
//			提交答案
			function answer(userId,jsonData,testDuration,teamTestId){
				var type = "";
				if (teamTestId!=null&&teamTestId!=""&&teamTestId!=0){
					type = "team";
				}
				$.ajax({
			        url: apiUrl+"/fxf_test/save_answer",
			        type: "post",
			        data: {userId:userId,type:type,testDuration:testDuration,answerJson:JSON.stringify(jsonData),teamTestId:teamTestId},
			        dataType: "json",
			        success: function(data) {
			        	if(data.success){
			                if(data.code == "2000"){
			                	var sclId = data.data.sclId;
			                	var resultShow = data.data.resultShow;
			                	
			                	if (isAndroid()) {
			                		window.android.toTest(data.code,resultShow);
			                	} else {
			                		toTest(data.code,resultShow);
			                	}
			                }else{
			                	if (isAndroid()) {
...
//			}
			...
			$(function(){
				...
				var teamTestId = getQueryString("teamTestId");
...
//							alert("请答完所有问题");
	...
	//						提交答案
							...
							answer(userId,jsonData,testDuration,teamTestId);
